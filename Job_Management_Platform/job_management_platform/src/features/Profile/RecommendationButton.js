...
export default function RecommendationButton({ r, refreshKeyHandler }) {
  const [openView, setOpenView] = useState(false)
  const [user, setUser] = useState(getUserId());
  const [error, setError] = useState();
  function remove(recoId){
    api.removeUserRecommendation(user, recoId) 
    ...
    <div >
      <div className="-mt-px py-6 flex divide-x divide-gray-200">
        <div className="flex w-0 flex-1">
          ...
          <button
            type="button"
            onClick={() => remove(r.recommendationId)}
            className="relative -mr-px inline-flex w-0 flex-1 items-center justify-center rounded-bl-lg border border-transparent py-4 text-sm font-medium text-gray-700 hover:text-gray-500"
          >
