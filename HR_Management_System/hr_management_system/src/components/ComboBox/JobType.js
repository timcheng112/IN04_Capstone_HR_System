  ...
  const filteredJobType =
    query === ''
      ...
      : jobTypes.filter((jobType) => {
          return jobType.name.toLowerCase().includes(query.toLowerCase())
        })

  return (
    <Combobox as="div" value={selectedJobType} onChange={setSelectedJobType}>
          ...
          <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            {filteredJobType.map((jobType) => (
              ...
              >
                {({ active, selected }) => (
                  <>
                    <span className={classNames('block truncate', selected && 'font-semibold')}>{jobType.name}</span>
