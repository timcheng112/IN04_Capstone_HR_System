...
export default function PromotionAddTeam({ open, onClose, teamHeadId }) {
  ...
  useEffect(() => {
    //console.log(teamHeadId);

    api.getDepartmentByEmployeeId(teamHeadId).then(response => {
  ...
  function addTeam() {
    api
      .addTeamPromotion(teamName, parseInt(teamHeadId), outlet, inOffice, parseInt(departmentId))
      .then((response) => {
