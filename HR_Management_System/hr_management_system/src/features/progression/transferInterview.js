...
export default function TransferInterview({ request }) {
  ...
  const handleSubmit = (evt) => {
    ...
    if (confirm) {
      api
        .conductTransferInterview(request.transferId, comments, "Passed")
        .then((response) => {
          console.log(response.data);
          alert(response.data);
        })
  ...
  function failInterview() {
    ...
    if (confirm) {
      api
        .conductTransferInterview(request.transferId, comments, "Failed")
        .then((response) => {
          console.log(response.data);
          alert(response.data);
        })
              ...
              <div>
                <div>
                  <h3 className="text-lg font-sans font-medium leading-6 text-gray-900">
                    ...
                    {request.employee.lastName}'s Transfer
                  </h3>
                </div>
