  ...
  const handleDelete = (evt) => {
    ...
    api.getAllGoalsByYear(year).then(response => {
      if (response.data.length) {
        alert('Cannot delete goal period. There have been ' + response.data.length + ' goal(s) added to ' + year + ".");
      } else {
        console.log('year ' + year)
        api.deleteGoalPeriod(year).then(response => {
          alert(response.data);
        })
      }
    })
                ...
                <div className="sm:flex-auto">
                  ...
                  <div className="mb-10 mx-36">
                    <div>
                      <label
                        htmlFor="period"
                        className="block text-lg mt-5 font-semibold text-gray-700"
                      >
                        Period
                      </label>
                      <select
                        id="period"
                        name="period"
                        className="mt-1 mr-5 font-semibold text-xl block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm bg-white"
                      >
