...
const tabs = [
  { name: "Tasks", href: "/admin/offboardinghr", current: false },
  {
          ...
          <div className="sm:flex sm:items-center space-x-4 mb-4">
            ...
            {user !== null && user.hrEmployee && (
              <button
                type="button"
                className="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto"
                onClick={() => history.push("/admin/offboarding")}
              >
                Non-HR Mode
              </button>
            )}
                        ...
                        {checklists.map((checklist, checklistIdx) => (
                          ...
                          >
                            ...
                            <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                              <ChecklistOptions checklist ={checklist} refreshKeyHandler={() => setRefreshKey((oldKey) => oldKey + 1)} isOnboarding={false}/>
                            </td>
                          </tr>
                        ))}
                      </tbody>
